...
public class Disassembler68000 {
	...
	private String decode(int opcode, int pc) {
	    ...
	    // --- Grupo 0x4000 (instruções diversas) ---
	    ...
	    if ((top & 0xF000) == 0x6000) {
	        ...
	        if (disp == 0 && pc + 2 < rom.length) {
	            disp = readWord(pc + 2); // word displacement
	        }
	        if ((disp & 0x8000) != 0) disp |= 0xFFFF0000; // sign extend
	        int target = pc + 2 + disp;
	        String[] conds = {
	            "BRA","BSR","BHI","BLS","BCC","BCS","BNE","BEQ",
	            "BVC","BVS","BPL","BMI","BGE","BLT","BGT","BLE"
	        };
	        if (cond < conds.length) return conds[cond] + " " + String.format("%06X", target);
	        return "B?? " + String.format("%06X", target);
	    }

	    // --- Grupo de MOVE (simplificado didático) ---
	    switch (top) {
	    ...
	    // fallback genérico, nunca deixa vazio
	    if ((top & 0xF000) == 0x4000) return "NEG/CLR";
