...
public class DisassemblerCPU68000Advanced02 {
	...
	public static String generateDisassembly(Cartridge cart) {
		...
		while (pc < rom.length) {
			...
			pc = instr.endsWith("...") ? rom.length
					: (instr.contains("##NEXT##") ? Integer.parseInt(instr.split("##NEXT##")[1]) : pc);
	...
	private static String decodeInstruction(int opcode, int pc, byte[] rom, int[] nextPc) {
		...
		if ((opcode & 0xFFF8) == 0x203C) {
			int reg = opcode & 0x7;
			if (pc + 3 >= rom.length)
				return "MOVE.L ??? (incompleto)";
			int imm = ((rom[pc] & 0xFF) << 24) | ((rom[pc + 1] & 0xFF) << 16) | ((rom[pc + 2] & 0xFF) << 8)
					| (rom[pc + 3] & 0xFF);
			nextPc[0] = pc + 4;
